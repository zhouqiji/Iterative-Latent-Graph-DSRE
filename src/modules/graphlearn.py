...
class GraphLearner(nn.Module):
    def __init__(self, input_size, hidden_size, topk=None, epsilon=None, num_pers=16, metric_type='attention',
                 device=None):
    ...
    def forward(self, context, ctx_mask=None):
        ...
        if ctx_mask is not None:
            attention = attention.masked_fill_(~ctx_mask.bool().unsqueeze(1), markoff_value)
            attention = attention.masked_fill_(~ctx_mask.bool().unsqueeze(-1), markoff_value)
    ...
    def build_knn_neighbourhood(self, attention, topk, markoff_value):
        ...
        weighted_adjacency_matrix = (markoff_value * torch.ones_like(attention)).scatter_(-1, knn_ind, knn_val).to(
            self.device)
